...
describe('Menu', () => {
  ...
  it('renders w/ selected', async () => {
    ...
    const selectedItem = el.querySelector('sp-menu-item[selected]') as MenuItem;
  ...
  it('has a "value" that can be copied during "change" events', async function () {
    ...
    const el = await fixture<Menu>(html`
      <sp-menu
        selects="single"
        @change=${({ target: { value } }: Event & { target: Menu }): void => {
  ...
  it('cleans up when tabbing away', async () => {
    ...
    expect(document.activeElement === firstItem, 'first item is active element')
      .to.be.true;
    expect(firstItem.focused, 'first item focused').to.be.true;
    el.dispatchEvent(arrowDownEvent());
    el.dispatchEvent(arrowDownEvent());
  ...
  it('can be controlled to manage a single togglable selection', async () => {
    const toggleSingleSelected = (event: Event & { target: Menu }): void => {
  ...
  it('handles long menu with basic selection', async () => {
    ...
    const lastItem = el.querySelector('sp-menu-item:nth-child(30)') as MenuItem;
  ...
  it('does not steal focus from input elements on mouseover', async () => {
    ...
    const textfieldInput = el.querySelector('#test-textfield') as HTMLElement;
    ...
    const nativeInput = el.querySelector('#test-native') as HTMLInputElement;
